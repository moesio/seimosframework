...
public class ProjectionPropertyAcessor implements PropertyAccessor {
	...
	private Method getterMethod(Class<?> theClass, String propertyName) {
		...
		for (Method method : methods) {
			if (method.getName().equals("get" + capitalize(property))
					|| method.getName().equals("is" + capitalize(property))) {
				result = method;
			}
		}

		if (result == null)
			throw new PropertyNotFoundException("Verifique a existência da propriedade " + propertyName + " na classe "
